...
| # | 시트 | 하는 일 | 언제 입력 |
|---|---|---|---|
| 1 | 설정 Settings | 브랜드·연락처, 소비자 계약·CASL 기간(10일 취소, 6개월 문의 동의 등), HST 등록 여부(YES/NO), EI 금액, 깔때기 가정(응답 35%·견적 10%·계약 40%, memo A8–A10), 비용 가정, 손익분기 입력, 게이트 상태, 킬 기준, 서비스 코드, 주별 목표, 가격표(읽기 전용) | 처음 한 번, 상태가 바뀔 때 |
...
| 7 | 지표 Dashboard | 전환율, 다음 주 문 목표, 남은 가구와 클러스터 #2 신호, 최근 7일 run-rate, 킬 기준 결과, DAILY 제목 줄 | 입력 없음 |
...
### 알아 둘 계산 규칙
- **시도(attempt):** NA+NI+Q+B+CB. 같은 집에 다시 가도 1회로 셉니다. NS(방문판매 금지 표시)는 두드리지 않으므로 빠집니다.
...
6. 점검일에는 Dashboard 5번 **킬 기준**을 봅니다: 10/4, 10/7, 10/9, 10/11, 10/18, 10/25, 10/27, 11/20.
...
- 공제 추정 = 순수입의 50% (주간 보험가능소득의 90%까지) + 그 위는 100%. EI 수령 추정 = B − 공제.
- **Path B 전환선** = (B − 0.5A + 0.881A) ÷ 0.881입니다 (A = Path A 주간 순수입, memo 5.4).
  - 최대 EI(B = $729)와 A = $400이면 약 $1,000/주가 됩니다.
  - 본인의 실제 B와 주간 보험가능소득을 설정 시트에 넣으면 다시 계산됩니다.
...
- 예시 가격은 `config/prices.ts`의 GTA 기본값에서 가져옵니다 (딥클린 2베드, 입주·이사 2베드, 일반 1베드). 실제 가격표와 같아야 합니다.
