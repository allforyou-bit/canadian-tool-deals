...
- "가정", "추정(ESTIMATE)"이라고 적힌 숫자는 근거 자료가 없는 값입니다. "확인 필요"는 조사에서 확인하지 못한 내용입니다.
...
- `crm.xlsx`는 빈 템플릿입니다. 실제 기록은 **복사본**(예: `내-CRM.xlsx`)이나 Google Sheets에서 하세요. 복사본은 **저장소 폴더 밖**(예: 내 문서)이나 git이 무시하는 `private/` 폴더에 두세요. 저장소 안 다른 곳에 두면 고객 정보가 GitHub에 올라갈 수 있습니다.
...
| 10 | 손익분기 Break-even | 홈통 청소(10건 / 보수 13건)와 제설(제설기 6 / 12, 삽만 3 / 6) 손익분기, 실제 계약 수 | 입력 없음 (실제 견적은 설정 시트) |
...
| 코드 | 뜻 | 다음에 할 일 |
|---|---|---|
| NA | 부재 (no answer) | 문고리 전단를 두고 나중에 다시 (시도 # +1) |
...
- 작업 시트에는 **분할금 1회마다** 한 줄씩 적습니다 (현장에서 확정한 시즌 요금 ÷ 4, 12/1, 1/1, 2/1, 3/1에 청구).
- 분할금을 EI에서 어느 주에 배분할지는 Service Canada에 확인하세요 (일한 주에 배분, EI Regs s.36(6)).
...
- **EI 주간신고 시트**는 일요일부터 토요일까지를 한 주로 봅니다 (EI Act s.2(1)의 week 정의).
- **대기 주간(첫 주)은 예외입니다.** 대기 주간에 번 소득은 50% 규칙이 아니라 첫 3주 수당에서 최대 전액까지 공제됩니다(EI Act s.19(1)). 시트의 50% 계산은 대기 주간 이후 주에만 쓰세요. 자세한 내용은 [03-EI-결정규칙.md](../03-EI-결정규칙.md).
...
- **Path B 전환선** = (B − 0.5A + 0.881A) ÷ 0.881입니다 (A = Path A 주간 순수입, memo 5.4).
  - 최대 EI(B = $729)와 A = $400이면 약 $1,000/주가 됩니다(추정). $729는 2차 자료와 법 조항 공식(68,900 ÷ 52 × 55%)으로 얻은 값입니다(memo F5). 실제 B는 Service Canada 금액으로 바꾸세요.
  - 본인의 실제 B와 주당 보험가입소득(Weekly insurable earnings)을 설정 시트에 넣으면 다시 계산됩니다.
...
- `ei-weekly.csv` 예시의 EI 숫자는 최대 EI($729, 2차 자료 + 법 조항 공식, memo F5) 기준 **추정**입니다.
...
5. 조건에 따라 아래 안내가 자동으로 붙습니다.
   - 방문 계약(Y): 온타리오 취소 안내. 계약서 사본을 받은 후 10일 이내에 취소 가능, 취소 통지를 받은 후 15일 이내에 환불 (F32, 검색 요약)
   - 제설 계약: 4회 분할 일정(현장에서 확정한 시즌 요금 ÷ 4, 12/1, 1/1, 2/1, 3/1), 12/1 전 결제 없음, 11/20까지 전체 제설 계약(모든 지역 합산)이 최소 건수에 이르지 않으면 무효, 11월 눈 요금은 계약서에서 선택한 경우에만 12/1 첫 분할금과 함께 청구
...
- 수신거부를 받으면 같은 날 하나뿐인 비공개 수신거부 목록(`business/b2b/unsubscribe-log.csv`를 저장소 폴더 밖이나 git이 무시하는 `private/` 폴더에 복사한 파일)에 한 줄 적고, 후속 시트에서도 "수신거부? Y"로 표시한 뒤 **10영업일 안에** 처리합니다. 문자·이메일을 보내기 전에는 항상 이 목록 하나를 확인합니다. 수신거부 방법은 60일 동안 유효해야 합니다 (F11).
...
- **T2125 지출 분류 목록은 조사 자료에 없습니다.** 최신 CRA T2125 양식에서 줄 번호와 이름을 확인하세요.
...
- 소득세율과 소득세 적립액은 조사 범위 밖이라 워크북에 없습니다.
- 차량 비용을 km당 얼마로 계산하는지는 조사하지 않았습니다. 주행 시트에는 기록만 남깁니다.


## 관문 G1 통과 전 홈통 예약 기록

문 두드리기 대본(`scripts/01-door-knock.md`)에 따라 G1 통과 전에 받은 홈통 예약은 **구두 예약**일 뿐입니다(서명·결제 없음). 문 기록(Doors)에는 결과 `B`로 적되 메모 칸에 `pre-G1`이라고 쓰고, 지표에서 계약(WON)으로 세지 않도록 G1이 통과된 날 확정하거나(10월 9일까지) 취소로 바꿉니다.
