...
2. 광고 없이 가장 그럴듯한 결과는 10월–1월 **4개월 합계 약 +C$8(판매 0건일 확률 약 75%)에서 +C$66**(1월에 판매 1건 정도)이에요(추정). 운이 아주 좋으면 약 +C$1,820이에요(추정). 단, 이 숫자들은 모두 한 번 사는 **Anthropic 크레딧값(약 C$7–14)을 빼기 전**이에요(추정). 크레딧값을 빼면 보수적인 경우(+C$8)는 **0 근처이거나 손해**이고, 판매가 0건이면 **산 크레딧값만큼 손해**예요(메모 §7.2 표와 그 바로 아래 줄).
...
단, 사이트는 **자기가 쓴 돈만** 세요. 7단계의 스테이징 점검과 "Grading eval"도 같은 크레딧을 쓰니, 운영에 처음 배포하기 직전에 Console 잔액으로 **한 번 더 적어요**(8-2 2번).
...
2. **크레딧 잔액 다시 적기 (5분, 산 게 없어도 해요):** 7단계의 스테이징 점검, "**Level-B checks**", "**Grading eval**"이 4-5에서 산 크레딧을 벌써 조금 썼어요(약 US$1–5, 추정). 그런데 운영 사이트는 `prepaidSince` 날짜부터 **자기가 쓴 돈만** 세요. 그래서 그대로 두면 4-5 ⑤에 적은 금액(예: US$10)이 **아직 다 남아 있다고** 믿어요.
   - Anthropic Console → **"Settings"** → "**Billing**"에서 **지금 잔액**을 봐요 [미확인: 메뉴 이름 — 4-5 ②에서 크레딧을 산 곳].
   - Claude에게 이렇게 말해요: "Console 잔액 US$○○야. 오늘 날짜로 크레딧 금액 다시 적어줘." (잔액은 비밀이 아니에요.)
   - Claude가 `ops/config/anthropic-limit.json`의 `prepaidUsd`(지금 잔액)와 `prepaidSince`(오늘, UTC 날짜)를 고치는 PR을 만들어요. **오너가 직접 병합**해요(2-2).
   - 이 PR을 병합한 **뒤에** 다음 번호로 가요. 그래야 첫 운영 배포가 진짜 잔액부터 세요.
3. 변수 `MPC_DEPLOY`를 새로 만들고 값 `true`를 넣어요.
4. **"Actions"** → **"Deploy practice coach"** → **"Run workflow"** → `target`: **`production`** → **"Run workflow"**.
5. 설정 검사 → 테스트 → 빌드 → 데이터베이스 표 적용 → 배포 → 점검(스모크 테스트) 순서로 돌아요. 실패하면 **자동으로 이전 버전으로 되돌려요**(첫 배포는 되돌릴 버전이 없어요).
6. 이제부터는 코드가 `master`에 합쳐질 때마다 **자동으로 배포**돼요.
7. **결제는 꺼진 상태로 시작해요**(`checkout_enabled` 기본값 false). 무료 연습과 무료 체험만 열려요.

> ✅ **이렇게 보이면 성공:** 초록색 ✓. 운영 주소가 열리고, 휴대폰으로 **"Continue with Google"** 로그인이 돼요. 사업용 Gmail로 온 알림이 있으면 스팸함 필터도 설정해요(4-3).
>
> ⚠️ **자주 하는 실수:**
> - 설정 검사 오류는 **빠진 값의 이름**을 알려 줘요(예: "Set the repository variable MPC_OWNER_EMAIL"). 그 이름을 5단계 표에서 찾아 넣고 다시 실행해요.
> - 2번(크레딧 잔액 다시 적기)을 건너뛰는 것. 사이트가 실제보다 크레딧이 많다고 믿어서, 무료 체험 끄기(70%)와 AI 피드백 멈춤(97%)이 늦게 오거나 오지 않아요. 그러면 크레딧이 먼저 떨어져 Anthropic이 채점을 거절할 수 있어요(9-3).
>
...
- [ ] 7단계 점검 뒤 **Console 잔액으로 크레딧 금액을 다시 적는 PR**을 병합했어요(8-2 2번). 그 뒤 "Grading eval"이나 스테이징 점검을 또 돌렸으면 한 번 더 적었어요(9-3).
...
2. **iPhone 말하기 (환불하기 전에):** 1번에서 산 이용권으로, iPhone Safari에서 말하기 과제 하나를 약 60초 녹음해 제출하고, 받아쓰기와 피드백이 오는지 봐요. AI 피드백 1회라 셀프 환불 조건(구매 뒤 AI 피드백 5회 이하) 안이에요. **5회를 넘기지 마세요.** 안 되면 iOS 버전과 증상만 Claude에게 알려요(녹음 파일은 보내지 않아요).
3. **셀프 환불 (말하기 점검 뒤에):** 계정 페이지 → **"Request a refund"**. Stripe에 환불이 보이고 이용권이 사라지는지 확인해요. 이 이메일과 카드로는 셀프 환불을 다시 할 수 없어요.
4. **다음 날:** **"Reconcile payments"** 최근 실행이 초록색(불일치 0건)인지 봐요.

> ✅ **이렇게 보이면 성공:** 2번에서 iPhone 말하기의 받아쓰기와 피드백이 오고, 3번 뒤에 Stripe에 환불이 보이고 계정 페이지에서 이용권이 사라져요. 다음 날 "**Reconcile payments**"가 초록색이에요.
>
> ⚠️ **자주 하는 실수:** 셀프 환불(3번)을 말하기(2번)보다 먼저 하는 것. 환불하면 이용권이 끝나서, 말하기 피드백이 무료 체험을 이미 썼다거나 지금은 쓸 수 없다는 안내로 거절될 수 있어요. iPhone 녹음이 고장 난 게 아니에요. **이용권을 다시 사지 마세요**(이 이메일과 카드로는 셀프 환불을 다시 할 수 없어요). Claude에게 먼저 물어요.
>
...
**그래서 이런 실행 뒤에는 잔액을 다시 적어요(산 게 없어도):** "Grading eval"(AI 채점 PR에서 **자동으로** 돈 것도 포함), 스테이징 점검(`target`이 `staging`인 배포 뒤에 AI 피드백을 받아 본 것), "Level-B checks"를 돌린 뒤에는 Anthropic Console의 **지금 잔액**을 봐요.

- **"Anthropic credits: top up"** 이슈가 열려 있으면 거기에 `잔액 US$8.40`처럼 잔액 한 줄만 댓글로 달아요(`구매` 줄은 없어도 돼요). 루틴이 만든 PR을 **오너가 직접 병합**해요. 루틴을 켠 뒤에는 새 "Grading eval"이 있을 때 `eval` 루틴이 이 이슈에서 잔액을 물어봐요.
- 이슈가 없거나 루틴(10단계)을 아직 켜지 않았으면 Claude에게 직접 말해요: "Console 잔액 US$○○야. 오늘 날짜로 크레딧 금액 다시 적어줘." Claude가 `prepaidUsd`(그 잔액)와 `prepaidSince`(오늘, UTC 날짜)를 고치는 PR을 만들면 **오너가 직접 병합**해요.

병합하면 사이트가 다시 배포되고 **진짜 잔액부터 다시 세요**. 안 적으면 70% 무료 체험 끄기와 97% 멈춤이 늦게 와서, 그 전에 크레딧이 떨어질 수 있어요(아래 "크레딧이 완전히 떨어지면").
...
시간: 약 15분 · **비용: 0원** (구매자에게 돌려주는 금액 말고는 없어요. 원래 결제의 Stripe 수수료는 돌려받지 못할 수 있어요 [미확인])
...
1. **결제 ID 찾기:** Stripe 대시보드 → "**Payments**"(결제) [미확인: 메뉴 이름] → 그 구매자의 결제를 눌러요. 결제 화면에서 `pi_`로 시작하는 **결제 ID**를 복사해요 [미확인: ID가 보이는 자리 — "Payment ID"나 "ID"라고 적힌 곳]. `ch_`나 `py_`로 시작하는 것은 다른 ID예요. 결제 ID와 금액은 **비밀이 아니에요.** 결제 하나에 이용권이 하나라서, 여러 번 산 구매자는 **결제마다 따로** 환불해요.
2. **남은 일수 찾기:** 남은 일수는 결제일이 아니라 이용권이 **끝나는 날**로 세요. 왜? 구매자가 이용권을 두 번 이상 샀으면 뒤의 이용권은 앞의 것이 끝난 **뒤에** 시작해요. AI 피드백이 멈췄던 적이 있으면(9-3) 이용권이 멈춘 시간만큼 늘어나서 끝나는 날이 뒤로 밀려요. 그래서 결제일로 세면 틀려요. 이용권 날짜는 사이트의 데이터베이스(D1)에 있어요.
   - Cloudflare 대시보드 → **"D1"** → 운영 데이터베이스 `mpc` → **"Console"** [미확인: 탭 이름 — 질문 문장(SQL)을 넣는 칸이 있는 탭]을 열어요.
   - 아래 줄을 **줄 전체** 복사해 붙여 넣어요. 맨 끝의 `pi_…`만 1번의 결제 ID로 바꿔요(작은따옴표 `'`는 남겨요). 그리고 실행해요 [미확인: 실행 버튼 이름 — "Execute"나 "Run"].

     `SELECT p.sku, p.starts_at, p.ends_at, p.revoked_at, MIN(CAST(julianday(date(p.ends_at)) - julianday(date('now')) AS INTEGER), CASE p.sku WHEN 'pass30' THEN 30 WHEN 'pass90' THEN 90 END) AS days_left FROM passes p JOIN purchases u ON u.id = p.purchase_id WHERE u.payment_intent = 'pi_…';`

   - 결과는 한 줄이에요: 이용권 종류(`sku`), 시작(`starts_at`), 끝(`ends_at`), 끝낸 때(`revoked_at`), 남은 일수(`days_left`). 이메일 같은 **개인정보는 없어요.** 날짜는 UTC(세계 표준시)예요.
   - **남은 일수 = `ends_at` 날짜 − 오늘 날짜**예요. 단, 이용권 일수(30 또는 90)보다 크게 세지 않아요. `days_left`가 이 계산을 대신 해 줘요.
   - `starts_at`이 오늘보다 뒤면 **아직 시작하지 않은 이용권**이에요. `days_left`가 이용권 일수 전체(30 또는 90)라서 **전액** 환불해요.
   - `revoked_at`에 날짜가 있으면 이미 끝난 이용권이에요(환불이나 분쟁). `days_left`가 0 이하이면 남은 날이 없어요. 둘 다 환불하지 않아요. 결과가 비어 있으면 결제 ID를 다시 확인해요.
   - 줄을 고치기 어려우면 Claude에게 "9-8의 D1 조회 줄을 결제 ID pi_○○로 만들어 줘"라고 해요. 이 결과에는 개인정보가 없어서 Claude에게 보내도 돼요.
3. **금액:** 결제한 금액 × 남은 일수 ÷ 이용권 일수(30 또는 90). 예: C$39 30일 이용권의 남은 12일 → 39 × 12 ÷ 30 = **C$15.60**. 미리보기 요약에도 이 계산표가 나와요.
4. **미리보기(`preview`)로 먼저 실행해요.** 저장소 → **"Actions"** 탭 → 왼쪽 목록의 "**Refund unused days**" → 오른쪽 **"Run workflow"** 버튼. 칸은 이렇게 채워요:
   - 브랜치: **`master`** 그대로
   - `payment_intent`: 1번에서 복사한 `pi_…` ID
   - `amount_cad`: 3번 금액을 **숫자와 점만** 써요. 예: `15.60` (`C$`, 쉼표, 빈칸 없이)
   - `mode`: **`preview`**(기본값)

   초록색 "**Run workflow**"를 눌러요. 잠시 뒤 목록 맨 위에 새 실행이 생겨요(제목 끝에 결제 ID가 보여요). 그 이름을 눌러 **요약(Summary)** 화면을 읽어요. 제목이 "**Refund unused days: 미리보기 (preview)**"이고, 표에 키 모드, 결제 ID, 결제일, 결제한 금액, 이미 환불된 금액, 환불할 수 있는 최대 금액, 요청 금액이 있어요. "무엇이 일어나나요"에는 "**이용권이 끝나요**"가 보여요. 이때는 **아직 아무것도 환불하지 않았어요.** 금액이 맞는지, 키 모드가 `live`(실사용)인지 확인해요. 요약의 결제일은 이용권이 가장 일찍 시작할 수 있는 날일 뿐이에요. 남은 일수는 2번의 `days_left`로 세요.
5. **진짜 환불(`refund`):** 요약이 맞으면 4번을 **같은 값으로** 다시 해요. 이번에는 `mode`만 **`refund`**(진짜 환불)로 바꿔요. 요약 제목이 "**환불했어요**"이고 환불 ID(`re_`로 시작)와 상태(`succeeded` 또는 `pending`)가 보이면 끝이에요. 사이트가 Stripe의 알림을 받으면 이용권을 **자동으로** 끝내요. 구매자에게 환불했다고 알려요.
6. `end_pass` 없이 부분 환불하면(예: 선의의 일부 환불) 이용권은 그대로예요. **전액 환불**은 메타데이터 없이도 이용권이 자동으로 끝나요. 그래서 전액 환불은 이 워크플로 없이 Stripe 대시보드의 그 결제 화면에서 "**Refund**" 버튼으로 해도 돼요 [미확인: 버튼 이름].
7. 환불이 나중에 실패하면 `[MPC] Refund failed` 알림이 와요. 구매자와 다른 환불 방법을 정해요.
8. **여러 구매자를 환불할 때**(예: 서비스 종료): 결제마다 1–5번을 해요. 다른 결제의 실행은 서로 기다리지 않고 서로 취소하지도 않아서, 한 구매자씩 이어서 눌러도 돼요.
...
> - 결제일부터 남은 일수를 세는 것. 2번의 `days_left`로 세요. 두 번째로 산 이용권은 아직 시작 전일 수 있어요(그러면 **전액** 환불).
...
> - 두 번 눌러도 **두 번 환불되지 않아요.** 같은 값의 두 번째 실행은 앞 실행이 끝날 때까지 기다렸다가 "이용권은 이미 끝났어요"라며 빨간색으로 멈춰요. 정상이에요.
> - 같은 결제로 짧은 시간에 세 번 이상 누르면, 기다리던 실행이 회색 "**Cancelled**"(취소) [미확인: 표시 이름]로 끝날 수 있어요. 그 실행은 아무것도 환불하지 않았어요. 앞 실행이 끝난 뒤 같은 값으로 다시 실행해요.
> - 요약 제목이 "**멈췄어요**"이면 아무것도 환불하지 않은 거예요(빨간색 ✗이어도 돈은 움직이지 않았어요). 요약에 이유가 적혀 있어요. 예: 금액이 최대 금액보다 커요, 분쟁(dispute) 중인 결제예요.
> - 요약에 "**새로 환불하지 않았어요**"가 있으면 Stripe가 전에 만든 환불을 다시 보여 준 거예요(같은 값의 요청에는 적어도 24시간 동안 같은 답을 줘요). 요약의 "지금 상태"가 `failed`나 `canceled`이면 돈이 돌아가지 않았어요. 구매자와 다른 환불 방법을 정하거나 24시간 뒤에 Claude에게 물어요.
> - 제목이 "**Stripe 오류**"인 실행이 같은 오류로 되풀이되면 계속 누르지 않아요. Stripe는 같은 값의 요청에 처음 준 답(오류도)을 적어도 24시간 동안 그대로 다시 보여 줘서, 다시 눌러도 같아요. 24시간 뒤에 하거나 Claude에게 물어요.
> - 요약의 키 모드가 `test`(시험)이면 진짜 돈은 움직이지 않아요. 실제 결제는 `live`예요.
>
> 💬 **막히면 Claude에게 이렇게 말하세요:** "Refund unused days가 멈췄어. 결제 ID는 pi_○○, 금액은 C$15.60, 실행 링크는 ○○야." (결제 ID, 금액, 실행 링크, 2번의 D1 조회 결과는 보내도 돼요. Stripe 키는 절대 보내지 마세요.)
