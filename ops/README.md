...
| `setup-cloudflare.yml` | manual only, once: "Set up Cloudflare (one time)" (input `target`: production, staging or both; default both). Safe to run again | for each target the D1 database named in `worker/wrangler.jsonc` (`mpc`, `mpc-staging`) and the FLAGS KV namespace exist: the id already in `worker/wrangler.jsonc`, else the one with the expected name, is reused, and one is created only when none exists (never a second one); the D1 migrations are applied to that database; the job summary shows the ids as a block that starts with "클라우드플레어 설정 결과야", which the owner sends to Claude, and as `wrangler.jsonc` entries. It changes no file: `deploy.yml` refuses a target until its ids are in `worker/wrangler.jsonc` | Cloudflare secrets (the API token needs D1 Edit and Workers KV Storage Edit) |
...
| `refund.yml` | manual only: "Refund unused days" (inputs `payment_intent` (`pi_…`), `amount_cad`, `mode`: preview or refund; default preview) | the pro-rated refund of a pass's unused days that also ends the pass (the Terms' promise; the Stripe Dashboard's refund dialog may not take metadata). Refuses a payment that is not a succeeded CAD payment, a disputed charge, more than what is left to refund, or a charge that already has a live `end_pass` refund; `preview` refunds nothing and writes the amounts and the pro-rated table to the job summary; `refund` creates the refund with metadata `end_pass=true` (the webhook then ends the pass) and the idempotency key `mpc-refund-<pi>-<cents>`, so a second run never refunds twice. Only ids and amounts are printed | `STRIPE_SECRET_KEY` |
...
(add `--env staging` for staging), or in the Cloudflare dashboard: Workers KV → the namespace `maple-practice-coach-flags`
(staging: `maple-practice-coach-staging-flags`; created by "Set up Cloudflare (one time)") → add or edit the keys [unverified: dashboard labels]. Setting `flag:grading_enabled` to `true` this way also ends
a pause the Worker started because Anthropic reported the credits used up (`auto:grading_off_credits`). The
Worker's next 15-minute check records a grading pause by itself, and its spend and prepaid-credit checks run on Cloudflare, so they keep working without Actions.
