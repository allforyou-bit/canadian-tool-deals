...
describe('POST /api/grade/speaking', () => {
  ...
  it('transcribes, grades the transcript, stores the transcript but never the audio', async () => {
    ...
    expect(row).toMatchObject({ kind: 'speaking', task_id: 'advice', input_text: TRANSCRIPT, audio_seconds: 42.5, refused: 0, free: 0 })
  ...
  it('refuses audio that turns out longer than the cap, recording the speech-to-text cost', async () => {
    ...
    expect(await gradeRowsFor(user.id)).toMatchObject([{ refused: 1, model: WHISPER_MODEL, audio_seconds: 600, cost_micro_usd: whisperCostMicroUsd(600) }])
  })
...
describe('free speaking sample', () => {
  ...
  it('is given back when no speech is detected, and the speech-to-text cost is still logged', async () => {
    ...
    expect(await gradeRowsFor(user.id)).toMatchObject([
      { refused: 1, free: 0, model: WHISPER_MODEL, audio_seconds: 12, cost_micro_usd: whisperCostMicroUsd(12), input_text: null },
    ])
  })
  ...
  it('is given back when transcription fails', async () => {
    ...
    expect(await freeSpeakingUsed(user.id)).toBe(0)
    expect(await gradeRowsFor(user.id)).toMatchObject([{ refused: 1, audio_seconds: 50, cost_micro_usd: whisperCostMicroUsd(50) }])
  })
  ...
  it('is given back when the grader output is unusable, with the full cost logged', async () => {
    ...
    expect(await freeSpeakingUsed(user.id)).toBe(0)
    expect(await gradeRowsFor(user.id)).toMatchObject([
      { refused: 1, free: 1, audio_seconds: 25, input_text: null, cost_micro_usd: whisperCostMicroUsd(25) + tokenCostMicroUsd('claude-opus-5', USAGE) },
    ])
  })
