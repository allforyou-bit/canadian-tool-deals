...
import { SESSION, SKUS, type Sku } from '../../../shared/config'
...
export class FakeStripe {
  ...
  private async handle(req: Request): Promise<Response> {
    ...
    if (key === 'POST /v1/refunds') {
      const pi = new URLSearchParams(body).get('payment_intent') ?? ''
      const amount = (this.paymentIntents.get(pi)?.amount as number | undefined) ?? 3900
      return Response.json({ id: `re_${uid()}`, object: 'refund', amount, status: this.refundStatus, payment_intent: pi })
    }
...
export async function startCheckout(user: User, sku: Sku = 'pass30'): Promise<string> {
  const res = await checkout(jsonRequest('/api/checkout', { sku, residentAttestation: true, lang: user.lang }), makeCtx(user))
  ...
  await env.DB.batch([
    env.DB.prepare(
      `INSERT INTO purchases (id, user_id, sku, amount_cents, currency, payment_intent, charge_id, card_fingerprint,
                              card_country, billing_country, billing_region, status, created_at, paid_at)
       VALUES (?1, ?2, ?3, ?4, 'cad', ?5, ?6, ?7, 'CA', 'CA', 'ON', 'paid', ?8, ?8)`,
