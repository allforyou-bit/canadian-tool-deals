...
| `deploy-config.ts` | deploy.yml configuration check: wrangler ids per target, site URL and custom domain, mailing address (required for production), grader variables, Anthropic Console limit, Stripe test keys on staging | `deploy.test.ts` |
