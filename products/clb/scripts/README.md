...
PAYMENT_INTENT=pi_… AMOUNT_CAD=15.60 MODE=preview|refund node scripts/run.mjs scripts/refund.ts   # refund.yml; needs STRIPE_SECRET_KEY
node scripts/run.mjs scripts/deploy-config.ts check --target production|staging     # deploy.yml
node scripts/run.mjs scripts/cloudflare-setup.ts --target production|staging|both   # setup-cloudflare.yml, once; needs Cloudflare
...
| `refund.ts` | "Refund unused days" (refund.yml): the pro-rated refund of a pass's unused days that also ends the pass. Reads the PaymentIntent (latest charge expanded) and the charge's refunds with the Worker's pinned Stripe-Version; refuses a payment that is not a succeeded CAD payment, a disputed charge, more than amount − amount_refunded, or a charge that already has a live `end_pass` refund. `preview` writes the key mode, ids, amounts and the pro-rated table (`metadata.sku` → SKUS days) to the job summary; `refund` POSTs /v1/refunds with metadata `end_pass=true` and Idempotency-Key `mpc-refund-<pi>-<cents>`. Prints ids and amounts only; the key and email-shaped text are redacted | `refund.test.ts` (mocked fetch; also checks refund.yml and guide section 9-8) |
| `deploy-config.ts` | deploy.yml configuration check (memo §7.2 Z3–Z6): wrangler ids per target (missing ids stop the deploy with a pointer to "Set up Cloudflare (one time)", also in the job summary), site URL and custom domain, the seller's legal name (required for production), mailing address (optional; required again with `MPC_LEARNER_EMAIL=on`), Google sign-in (`MPC_GOOGLE_CLIENT_ID` + `GOOGLE_CLIENT_SECRET`, required for production), `MPC_OWNER_EMAIL` (required for production), the sender (default: Resend's sandbox `onboarding@resend.dev`, refused with `MAGIC_LINK=all` or `LEARNER_EMAIL=on`), `MAGIC_LINK` / `LEARNER_EMAIL` (always passed; default `owner` / `off`), a real Turnstile site key and secret (required for production), grader variables, Anthropic limit (production: `monthlyLimitUsd`; staging: `evalMonthlyLimitUsd`) and the prepaid-credit ledger (production, only when `prepaidUsd` and `prepaidSince` are both valid), Stripe test keys on staging, the staging lock (`STAGING_ALLOWED_EMAILS` from `MPC_STAGING_ALLOWED_EMAILS` or `MPC_OWNER_EMAIL`; never in production) and the `STAGING - ` sender; `live-version` reads the version at 100% from `wrangler deployments status --json` (before the deploy and before a rollback) | `deploy.test.ts` |
| `cloudflare-setup.ts` | the one-time workflow "Set up Cloudflare (one time)" (setup-cloudflare.yml): per target, finds the D1 database (`database_name` in wrangler.jsonc: `mpc`, `mpc-staging`) and the FLAGS KV namespace (`<Worker name>-flags`; a FLAGS / FLAGS_STAGING namespace from the earlier manual guide is reused) by the id in wrangler.jsonc or by name, creates one only when none exists, applies the D1 migrations to exactly that database (a temporary config with its id) and writes the ids to the job summary: a block starting with "클라우드플레어 설정 결과야" for the owner to send to Claude, and the `d1_databases` / `kv_namespaces` entries for wrangler.jsonc. Never takes the other target's resource; redacts the account id from wrangler output. deploy-config.ts refuses a deploy until the ids are in wrangler.jsonc and names this workflow | `cloudflare-setup.test.ts` (fake wrangler with an in-memory account) |
...
network; the D1 and Stripe readers are tested with recorded output shapes, and the Cloudflare setup with a fake
wrangler (output shapes from wrangler 4.137.0's code). `tsc` also fails when
