...
- 마케팅 동의 체크가 없는 고객: 법으로는 문의일로부터 6개월간 묵시적 동의가 있을 수 있지만(CASL s.10(10), 결정 메모 F11), **이 사이트의 개인정보 안내·FAQ·계약서는 별도 동의 없이는 홍보 메시지를 보내지 않는다고 약속합니다.** 그러니 동의 체크가 없는 고객에게는 견적·일정 등 요청에 대한 답만 보내고 홍보성 메시지는 보내지 마세요. 모르는 소비자에게 먼저 문자·이메일을 보내는 것(콜드 메시지)은 하지 않습니다.
